...
import expressionParser.EvaluationException;
import expressionParser.ExpressionNode;
import expressionParser.Parser;
import expressionParser.ParserException;
import expressionParser.SetVariable;

import java.util.ArrayList;
import java.util.HashMap;

public class MainAllocationProcessContainer_IndTest {
	...
	// Constructor
	public MainAllocationProcessContainer_IndTest(ContainerAllocationProblem containerAllocationProblem,
			EvolutionState state, double vmCpuOverheadRate, double vmMemOverhead) {
	...
	public ArrayList<Double> test(DoubleData input, EvolutionState state, ArrayList<ArrayList<Double[]>> inputX,
			...
			ArrayList<Double[]> pmTypeList, ArrayList<Double[]> vmTypeList, String vmSelectionCreationRule,
			String vmAllocationRule, int threadnum, ADFStack stack) {
			...
			if (chosenVM < currentVmNum) {
				...
				if (chosenPM >= pmResourceList.size()) {
					if (this.newPMFlag) { // if this is a new pm
						pmResourceList.add(new Double[] { this.newPMHolder[0] - vmTypeList.get(vmType)[0],
								this.newPMHolder[1] - vmTypeList.get(vmType)[1], this.newPMHolder[2],
								this.newPMHolder[3], this.newPMHolder[4], this.newPMHolder[5] });
					}

					// Add the Actual usage to the PM
					// Here, we must consider the overhead
					pmActualUsageList.add(new Double[] {
							this.newPMHolder[0] - containerCpu - vmTypeList.get(vmType)[0] * vmCpuOverheadRate,
							this.newPMHolder[1] - containerMem - vmMemOverhead, this.newPMHolder[2],
							this.newPMHolder[3], this.newPMHolder[4], this.newPMHolder[5] });
					...
					this.currentEnergyUnitTime += (this.newPMHolder[3] - this.newPMHolder[2])
...
//        System.out.println(this.energyConsumption);
	...
	private Integer VMAllocation(DoubleData input, final EvolutionState state, String ind, final int threadnum,
			final ADFStack stack, ArrayList<Double[]> pmResourceList, ArrayList<Double[]> pmActualResourceList,
			ArrayList<Double[]> pmTypeList, double vmCpuCapcacity, double vmMemCapacity, double vmUsedCpu,
			double vmUsedMem, double vmCore) {
		...
		for (Double[] pm : tempPMResourceList) {
			...
			if (pmCpuRemain >= vmCpuCapcacity && pmMemRemain >= vmMemCapacity) {
...
//				if (chosenPM == null || pmScore > BestScore) {
		...
		if (chosenPM == null) {
			System.err.println("No suitable PM for the VM!Something must be wrong in filtering containers");
		}
		...
		if (chosenPM >= pmResourceList.size()) {
			...
			this.newPMHolder = new Double[] { tempPMResourceList.get(chosenPM)[0], // cpu
					...
					tempPMResourceList.get(chosenPM)[2], tempPMResourceList.get(chosenPM)[3],
					tempPMResourceList.get(chosenPM)[4], tempPMResourceList.get(chosenPM)[0] };
		}

		return chosenPM;
	}

	private Double EvolveVmAllocationMethod(final DoubleData input, final EvolutionState state, String ind,
		...
		ExpressionNode expr = parser.parse(ind);
		try {
			expr.accept(new SetVariable("VmCPU", myEvolutionState.normalizedVmCpuCapacity));
			expr.accept(new SetVariable("vmMem", myEvolutionState.normalizedVmMemCapacity));
			expr.accept(new SetVariable("PmCPU",
					myEvolutionState.normalizedPmCpuRemain - myEvolutionState.normalizedVmCpuCapacity));
			expr.accept(new SetVariable("pmMem",
					myEvolutionState.normalizedPmMemRemain - myEvolutionState.normalizedVmMemCapacity));
			expr.accept(new SetVariable("NoOfCore", myEvolutionState.coreNumber));
			
			expr.accept(new SetVariable("PmUSedCPU", myEvolutionState.normalizedPmActualCpuUsed));
			expr.accept(new SetVariable("PmUsedMem", myEvolutionState.normalizedPmActualMemUsed));
...
//			System.out.println("The value of the expression is " + expr.getValue());
	...
	private Double EvolveSelectionCreationMethod(DoubleData input, final EvolutionState state, String ind,
		...
		try {
			expr.accept(new SetVariable("leftVmCpu",
					myEvolutionState.normalizedVmCpuRemain - myEvolutionState.normalizedContainerCpu));
			expr.accept(new SetVariable("leftVmMem",
					myEvolutionState.normalizedVmMemRemain - myEvolutionState.normalizedContainerMem));
			expr.accept(new SetVariable("coCpu", myEvolutionState.normalizedContainerCpu));
			expr.accept(new SetVariable("coMem", myEvolutionState.normalizedContainerMem));
			expr.accept(new SetVariable("vmCpuOverhead", myEvolutionState.normalizedVmCpuOverhead));
			expr.accept(new SetVariable("vmMemOverhead", myEvolutionState.normalizedVmMemOverhead));
...
//			System.out.println("The value of the expression is " + expr.getValue());
	...
	private Integer VMSelectionCreation(final DoubleData input, final EvolutionState state, String ind,
		...
		for (Double[] vm : tempVMResourceList) {
			...
			if (vmCpuRemain >= containerCpu && vmMemRemain >= containerMem && vmOS == containerOS) {
				...
				Double vmScore = EvolveSelectionCreationMethod(input, state, ind, threadnum, stack, vmCpuRemain,
						vmMemRemain, vmTypeList.get(vmType)[0], vmTypeList.get(vmType)[1], pmResourceList,
...
//		System.out.println("newUtil: "+ newUtil);
